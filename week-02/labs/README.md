...
1. Distinguish operational signals, KPIs, and a North Star — and explain how a change in one is supposed to ripple to the others.
2. Propose a North Star metric for a product and defend it against three standard pitfalls (vanity metric, leading-indicator confusion, gameable targets).
3. Apply core UX heuristics (Nielsen's 10, accessibility minimums) to audit a real product surface and produce a short list of design principles a TPM-engineer pair can ship against.
4. Use AI to compile a multi-source research summary with evidence tagging, and detect when the AI is over-confident relative to its inputs.
5. Build a customer journey map that connects observed pain points (from Week 1) to a prioritized list of feature concepts.
...
- `day-02-north-star.md` — North Star pitfalls card, candidate-evaluation rubric
- `day-03-design-ux.md` — UX heuristics card, audit template
- `day-04-ai-research-summaries.md` — Strategy brief template, evidence-tag schema
- `day-05-journey-mapping.md` — Journey map canvas, feature-prioritization frame
...
| Design awareness | 10% | At least one UX principle is named and its trade-off acknowledged |
