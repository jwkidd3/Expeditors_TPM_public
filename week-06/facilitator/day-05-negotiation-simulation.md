...
- Post sim pairings at 09:00: pair quad A with quad B (and so on). Each quad plays "stakeholder" for one of their counterpart's negotiations.
- Have the three role-play card sets ready to hand to the **stakeholder-side** quad each round (do not give them to the author side):
...
**How to run the surprise:** fresh pairing; one quad plays an executive (CFO / GM) from `week-06/handouts/executive-demand-scenario-cards.md`. The executive opens with the **demand only** and offers no give — the author must find the trade space. Each card's Hidden section carries the real driver and the actual settle point; do not reveal it — make the author find the real constraint. Agree somewhere by the end (a hard executive who never yields teaches nothing). Force the close: no round ends without an owner + next step. The answer-key content per card:
...
- Read aloud the **best line of negotiation** from each quad.
...
- Which quad held the line most credibly? They are the Week-7 positive example.
- Which quad **discovered** the trade space rather than improvising? Hold up Friday.
- Did anyone have a moment of "I wish I'd said X" that they should rehearse for next time?
- Which quad's outcomes log is most actionable — owners, dates, specifics? That's the senior signal.
